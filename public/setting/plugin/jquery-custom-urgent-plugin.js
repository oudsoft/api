...
(function ( $ ) {
  ...
  $.fn.customurgent = function( options ) {
    ...
    const modalContentWrapperClass = {"margin": "10% auto", "width": "45%", "background-color": "white"};
    const modalHeaderClass = {"width": "100%", "height": "auto", "background-color": "#184175", "color": "white", "padding": "4px", "text-align": "left"};
    const modalContentClass = {"width": "100%", "height": "auto", "background-color": "white", "padding": "4px", "text-align": "left"};
    const modalFooterClass = {"font-size": "20px", "width": "100%", "height": "auto", "background-color": "#184175", "padding": "4px", "text-align": "center"};
    ...
    const doCreateUrgentForm = function(){
      ...
      let acceptStepLabel = $('<div><b>ระยะเวลาตอบรับ ' + settings.urgentOf + '</b></div>');
      ...
      if (settings.useWorkingStep) {
        ...
        let workingStepOption = $('<input type="checkbox" id="WorkingStepOption" value="0" style="transform: scale(1.5);"><label for="WorkingStepOption"> ใช้ค่าเดียวกับระยะเวลาตอบรับเคส</label>');
    ...
    const doCreateModalContent = function(){
      let modalWrapper = $('<div id="CustomUrgentDialog"></div>');
      $(modalWrapper).css(modalContentWrapperClass);
      //$(modalWrapper).css(settings.externalStyle);
      let modalHeader = $('<div><h3>กำหนดเวลา' + settings.urgentWord + '</h3></div>');
      $(modalHeader).css(modalHeaderClass);
      $(modalWrapper).append($(modalHeader));

      let modalContent = $('<div id="ModalContent"></div>');
      let guideBox = $('<divstyle="width: 100%; padding: 10px; margin-top: 10px; background: #ddd;>โปรดระบุวัน-เวลาที่จะใช้กำหนดเวลา' + settings.urgentWord + ' จากรังสีแพทย์</div>');
      ...
      $(okCmd).on('click', async (evt)=>{
        ...
        if (settings.useWorkingStep) {
          ...
          if ((critiriaMinuteA >= 15) && (critiriaMinuteW >= 15)) {
            ...
            eventData = {Accept: {dd: ddA, hh: hhA, mn: mnA, text: resultA}, Working: {dd: ddW, hh: hhW, mn: mnW, text: resultW}};
            settings.successCallback(eventData);
            $(modalWrapper).trigger('closedialog', [eventData]);
          } else {
            alert('ระยะเวลาตอบรับเคสและระยะเวลาส่งผลอ่านต้องมีค่าล่วงหน้าจากตอนนี้ไปอีกอย่างน้อย 15 นาที');
          }
        } else {
          eventData = {Accept: {dd: (ddA), hh: hhA, mn: mnA, text: resultA}};
          settings.successCallback(eventData);
          //$(modalWrapper).trigger('closedialog', [eventData]);
        }
        /*
          dd: (ddA-1) กับ dd: (ddA)
          ข้างบนต้องลบ 1 เพราะมีการเปรียบเทียบระหว่าง Accept หับ Working
        */
      });
      return $(modalWrapper);
    }
    ...
    const doCloseDialog = function() {
      let overlay = $('body').find('#Custom-Urgent-Dialog');
      let mainModal = $('body').find('#CustomUrgentDialog');
      $(mainModal).remove();
      $(overlay).remove();
    }
    ...
    let output = {
      ...
      doCalNewTime: function(dd, hh, mn){
        doCalNewTime(dd, hh, mn)
      },
