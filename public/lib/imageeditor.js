...
(function($) {
  $.fn.imageeditor = function( options ) {
    ...
    const doSaveCaptureImage = function(e){
      ...
      $.ajax({
        ...
        data: formData}).done(function(data){
          ...
          //let previewBox = doCreatePreviviewDialogBox(dataURL);
          /*
          let previewBox = doCreatePreviviewDialogBox(data.link);
          $('#PopupPreview').css(quickReplyDialogStyle);
          $(previewBox).css(quickReplyContentStyle);
          $('#PopupPreview').append($(previewBox));
          */
          let imgFilePaths = data.link.split('/');
          let imgFileNames = imgFilePaths[imgFilePaths.length-1].split('.');
          let id = imgFileNames[0];
          let sharePageUrl = '/share/?id=' + id;
    ...
    const doCreatePreviviewDialogBox = function(imgUrl){
      ...
      */
      ...
      let imgSource = 'https://radconnext.info' + imgUrl;
      ...
      //let lineShareContent = $('<a href="https://social-plugins.line.me/lineit/share?url=https://radconnext.info'+ imgUrl +'"></a>');
      let imgFilePaths = imgUrl.split('/');
      let imgFileNames = imgFilePaths[imgFilePaths.length-1].split('.');
      let id = imgFileNames[0];
